...
export function RequestsPage({ workspaceID = "" }: { workspaceID?: string }) {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      return [item.id, item.title, item.workspace_id, item.target_tool, item.source, item.tunnel_id, item.tunnel_name, item.status, item.session_hash].some((value) => value?.toLowerCase().includes(needle))
    })
  }, [items, query, status])
...
function tunnelLabel(item: { tunnel_name?: string; tunnel_id?: string }) { return item.tunnel_name || item.tunnel_id || "" }
