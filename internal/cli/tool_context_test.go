...
func TestMCPToolContextAllowsOnlyReadOnlyCLICommands(t *testing.T) {
	...
	root := newRootCommand()
	for _, path := range [][]string{{"status"}, {"config", "list"}, {"config", "preset", "show"}, {"auth", "status"}, {"cluster", "status"}, {"workspace", "access", "list"}, {"mcp", "server", "show"}, {"tunnel", "status"}} {
	...
	for _, path := range [][]string{{"serve"}, {"up"}, {"down"}, {"_service", "run"}, {"logs", "clear", "--force"}, {"config", "set"}, {"config", "reload"}, {"config", "convert"}, {"auth", "mcp", "create"}, {"cluster", "relay"}, {"workspace", "access", "add"}, {"mcp", "server", "add"}, {"tunnel", "enable"}} {
		cmd, _, err := root.Find(path)
		if err != nil {
			t.Fatal(err)
		}
