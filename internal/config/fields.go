...
var fieldSpecs = []FieldSpec{
	...
	{Key: "auth.mcp_enabled", Label: "MCP authentication", Section: FieldSectionAccess, Description: "controls token authentication for the MCP HTTP endpoint", Details: "When the MCP HTTP server is enabled and this setting is true, an MCP credential must be configured. Non-loopback HTTP exposure requires MCP authentication to be enabled with a configured credential.", Kind: FieldBool, Editable: true, Related: []string{"auth.mcp_token_hash", "server.enabled", "server.expose.mode"}},
	{Key: "auth.admin_enabled", Label: "Admin authentication", Section: FieldSectionAccess, Description: "controls token authentication for the admin HTTP endpoint", Details: "When the admin server is enabled and this setting is true, an admin credential must be configured. Non-loopback exposure with the admin endpoint enabled requires admin authentication.", Kind: FieldBool, Editable: true, Related: []string{"auth.admin_token_hash", "admin.enabled", "server.expose.mode"}},
	...
	{Key: "shell.approval_policy", Label: "Approval policy", Section: FieldSectionShell, Description: "controls when shell commands require local approval", Details: "Explicit deny rules take precedence over allow rules. The selected policy then determines whether commands that are not covered by an explicit rule require local approval. Independent security guards can still reject commands even when approval is not required.", Kind: FieldEnum, Options: []string{"allow", "balanced", "strict", "deny"}, Values: []FieldValueSpec{{Value: "allow", Description: "Run commands without approval unless an explicit deny rule matches."}, {Value: "balanced", Description: "Require approval for guarded or risky operations and external access."}, {Value: "strict", Description: "Require approval for execution that is not recognized as a workspace-confined static read."}, {Value: "deny", Description: "Require approval for commands unless an explicit allow rule matches."}}, Editable: true, Related: []string{"shell.approval_allow_commands", "shell.approval_deny_commands", "shell.environment_policy", "shell.sandbox_policy", "shell.network_policy"}},
