...
var fieldSpecs = []FieldSpec{
	...
	{Key: "shell.execution_feed_max_bytes", Label: "Command execution event buffer", Section: FieldSectionShell, Description: "sets the maximum retained command execution event data", Details: "The runtime retains recent command execution events up to this byte budget for Command Execution replay. The default is 1 MB. Valid values are 64000-100000000 bytes and changes are applied to the running runtime without a restart.", Kind: FieldInt, Editable: true, Guidance: "Use the Command Execution scope editor for preset sizes or human-readable custom values such as 5 MB."},
...
func SetValue(cfg *Config, key, raw string) error {
	...
	case "shell.execution_feed_max_bytes":
		value, err := parseIntField(raw, key)
		if err != nil {
			return err
		}
		cfg.Shell.ExecutionFeedMaxBytes = value
...
func RawValue(cfg Config, key string) (string, error) {
	...
	case "shell.execution_feed_max_bytes":
		return strconv.Itoa(cfg.Shell.ExecutionFeedMaxBytes), nil
	case "features.ponytail.active":
