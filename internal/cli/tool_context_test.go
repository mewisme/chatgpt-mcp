...
func TestMCPToolContextAllowsOnlyReadOnlyCLICommands(t *testing.T) {
	...
	for _, path := range [][]string{{"install"}, {"update"}, {"serve"}, {"up"}, {"down"}, {"_service", "run"}, {"logs", "clear", "--force"}, {"config", "set"}, {"config", "reload"}, {"config", "convert"}, {"config", "export"}, {"config", "import"}, {"auth", "mcp", "create"}, {"request", "approve"}, {"request", "deny"}, {"workspace", "access", "add"}, {"mcp", "server", "add"}, {"tunnel", "enable"}, {"alias", "install"}, {"alias", "remove"}} {
		cmd, _, err := root.Find(path)
		if err != nil {
			t.Fatal(err)
		}
