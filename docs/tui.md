...
`Command Execution` reuses the same bounded globally ordered execution feed produced by the runtime. It replays recent events before continuing live and renders stdout and stderr in event order instead of splitting them into separate panels. `START`, `CONTINUE`, and `END` boundaries keep concurrent executions readable when their output interleaves; headers include execution/workspace identity, local time, source, safe session fingerprint, call ID, routing metadata, cwd, and command when available, while completion includes status, exit code, and duration. Raw MCP session IDs are never shown.
