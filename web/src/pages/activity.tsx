...
export function ActivityPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return events.filter((event) => {
      ...
      return [event.kind, event.method, event.source, event.tool, event.workspace_id, event.session_hash, event.session_binding, event.session_workspace_id, event.received_by_instance_id, event.executed_by_instance_id, event.status, event.message].some((value) => value?.toLowerCase().includes(needle))
    })
