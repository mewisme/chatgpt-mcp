...
func compile(repoRoot, goos, goarch, output string) error {
	cmd := exec.Command("go", "build", "-trimpath", "-ldflags", "-s -w", "-o", output, "./plugins/ponytail/cmd/ponytail")
	cmd.Dir = repoRoot
...
func zipFile(sourcePath, entryName, outputPath string) error {
	temp, err := os.CreateTemp(filepath.Dir(outputPath), ".ponytail-plugin-*.zip")
	if err != nil {
		return err
	}
	tempPath := temp.Name()
