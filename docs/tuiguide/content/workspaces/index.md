...
If the project directory has already been renamed or moved, press `m` from the workspace detail (or choose **Relocate** from Commands). Relocate opens a routed full-page directory editor. Pressing `Enter` on its final field rebinds the existing workspace to that directory; it does not move project files. Workspace identity is preserved. Container membership and `.cgm` state stay with the project.

Unregister removes the workspace from the local index without deleting project files or `.cgm` and therefore requires confirmation. **Delete local state** (`D`) unregisters the workspace and deletes `<workspace>/.cgm`.
