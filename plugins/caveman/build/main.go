...
func main() {
	...
	flag.StringVar(&templatePath, "template", "plugins/caveman/plugin.json", "plugin manifest template")
...
func compile(repoRoot, goos, goarch, output string) error {
	cmd := exec.Command("go", "build", "-trimpath", "-ldflags", "-s -w", "-o", output, "./plugins/caveman/cmd/caveman")
	cmd.Dir = repoRoot
...
func zipFile(sourcePath, entryName, outputPath string) error {
	temp, err := os.CreateTemp(filepath.Dir(outputPath), ".caveman-plugin-*.zip")
	if err != nil {
		return err
	}
	tempPath := temp.Name()
