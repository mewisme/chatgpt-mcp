...
func TestWindowsTaskIsPerUserLeastPrivilege(t *testing.T) {
	...
	for _, expected := range []string{"<LogonType>InteractiveToken</LogonType>", "<RunLevel>LeastPrivilege</RunLevel>", "<RestartOnFailure>", "--config-dir", `C:\Users\Mew\.config\chatgpt-mcp`, "<Interval>PT1M</Interval>"} {
