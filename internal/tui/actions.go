...
func systemActions() []action.Action {
	return []action.Action{
		...
		systemAction("auth.mcp.rotate", "Rotate Direct MCP HTTP token", "Generate a new Direct MCP HTTP token. Reuse the current token when adding this MCP server to ChatGPT.", []string{"auth", "mcp", "token", "rotate", "create", "direct"}, []string{"auth", "mcp", "create"}, tuipage.AuthMCPRotate, false),
