...
func main() {
	...
	flag.StringVar(&templatePath, "template", "plugins/cf-tunnel/plugin.json", "plugin manifest template")
...
func compile(repoRoot, goos, goarch, output string) error {
	cmd := exec.Command("go", "build", "-trimpath", "-ldflags", "-s -w", "-o", output, "./plugins/cf-tunnel/cmd/cf-tunnel")
	cmd.Dir = repoRoot
...
func zipFile(sourcePath, entryName, outputPath string) error {
	temp, err := os.CreateTemp(filepath.Dir(outputPath), ".cf-tunnel-plugin-*.zip")
	if err != nil {
		return err
	}
	tempPath := temp.Name()
