...
func TestPluginTemplateValidatesWithSentinelDigests(t *testing.T) {
	...
	if manifest.ID != "secure-mcp-tunnel" || manifest.Type != "runtime" {
...
func TestBuildCurrentPlatform(t *testing.T) {
	...
	template := filepath.Join(repoRoot, "plugins", "secure-mcp-tunnel", "plugin.json")
	...
	manifestPath, err := pluginbuild.Build(pluginbuild.Request{
		...
		Package: "./plugins/secure-mcp-tunnel/cmd/secure-mcp-tunnel",
	})
	if err != nil {
		t.Fatal(err)
	}
