...
func TestPluginTemplateValidatesWithSentinelDigests(t *testing.T) {
	...
	if manifest.ID != "markdown-formatter" || manifest.Type != "formatter" {
...
func TestBuildCurrentPlatform(t *testing.T) {
	...
	template := filepath.Join(repoRoot, "plugins", "markdown-formatter", "plugin.json")
	...
	manifestPath, err := pluginbuild.Build(pluginbuild.Request{
		...
		Package: "./plugins/markdown-formatter/cmd/markdown-formatter",
	})
	if err != nil {
		t.Fatal(err)
	}
