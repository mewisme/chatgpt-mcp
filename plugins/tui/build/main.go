...
func main() {
	...
	flag.StringVar(&templatePath, "template", "plugins/tui/plugin.json", "plugin manifest template")
...
func compile(repoRoot, goos, goarch, output string) error {
	cmd := exec.Command("go", "build", "-trimpath", "-ldflags", "-s -w", "-o", output, "./plugins/tui/cmd/tui")
	cmd.Dir = repoRoot
...
func zipFile(sourcePath, entryName, outputPath string) error {
	temp, err := os.CreateTemp(filepath.Dir(outputPath), ".tui-plugin-*.zip")
	if err != nil {
		return err
	}
	tempPath := temp.Name()
