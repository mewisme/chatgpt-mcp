...
func TestWindowsTaskIsPerUserLeastPrivilege(t *testing.T) {
	...
	for _, expected := range []string{"<LogonType>InteractiveToken</LogonType>", "<RunLevel>LeastPrivilege</RunLevel>", "<RestartOnFailure>", "<Command>powershell.exe</Command>", "-WindowStyle Hidden", "--config-dir", `C:\Users\Mew\.config\chatgpt-mcp`, "<Interval>PT1M</Interval>"} {
		if !strings.Contains(xml, expected) {
			t.Fatalf("task XML missing %q:\n%s", expected, xml)
		}
	}
	for _, forbidden := range []string{"SYSTEM", "HighestAvailable", "Password", "encoding=", "<Interval>PT3S</Interval>"} {
