...
import (
	...
	corecaveman "go.mewis.me/chatgpt-mcp/internal/caveman"
	"go.mewis.me/chatgpt-mcp/internal/toolprovider"
)

type Handler struct {
	manager *corecaveman.Manager
}

func NewHandler() *Handler {
	return &Handler{manager: corecaveman.NewManager(true, corecaveman.Full)}
}
...
func (h *Handler) Describe(context.Context, json.RawMessage) (any, error) {
	return toolprovider.DescribeResult{Tools: []toolprovider.Tool{{
		Name: "caveman_turn", Title: "Caveman Turn Controller",
		Description:  "Caveman controller. Call before each user-facing response; configured active/mode values seed each workspace state. Pass the exact current user prompt.",
		...
		OutputSchema: json.RawMessage(`{"type":"object","properties":{"available":{"type":"boolean"},"mode":{"type":"string","enum":["off","lite","full","ultra","wenyan-lite","wenyan-full","wenyan-ultra"]},"active":{"type":"boolean"},"active_instructions":{"type":"string"},"refresh_hint":{"type":"string"}},"required":["available","mode","active"],"additionalProperties":false}`),
...
func (h *Handler) Reconcile(_ context.Context, params json.RawMessage) (any, error) {
	...
	h.manager.SetDefaults(boolSetting(reconcile.Settings, "default_active", true), corecaveman.Mode(stringSetting(reconcile.Settings, "default_mode", "full")))
	return map[string]bool{"ok": true}, nil
}
