...
const columns = columnHelper.columns([
  ...
  columnHelper.display({ id: "tunnel", header: "Tunnel", cell: ({ row }) => { const label = tunnelLabel(row.original); return label ? <TruncatedText className="text-xs text-muted-foreground" lines={1}>{label}</TruncatedText> : <span className="text-muted-foreground">-</span> } }),
...
export function ActivityPage() {
  ...
  const tunnels = useMemo(() => unique(events, pending, (event) => event.tunnel_id), [events, pending])
  const tunnelLabels = useMemo(() => {
    const labels: Record<string, string> = {}
    for (const event of [...events, ...pending]) {
      if (event.tunnel_id && !labels[event.tunnel_id]) labels[event.tunnel_id] = tunnelLabel(event) || event.tunnel_id
    }
    return labels
  }, [events, pending])
  const filtered = useMemo(() => {
    ...
    return events.filter((event) => {
      ...
      if (tunnel !== "all" && event.tunnel_id !== tunnel) return false
      if (!needle) return true
      return [event.kind, event.method, event.source, event.tunnel_id, event.tunnel_name, event.tool, event.workspace_id, event.session_hash, event.session_access, event.session_workspace_count?.toString(), event.received_by_instance_id, event.executed_by_instance_id, event.status, event.message].some((value) => value?.toLowerCase().includes(needle))
    })
  }, [events, kind, query, source, status, tunnel])
...
function ActivityMobileList({ events, onSelect }: { events: ActivityEvent[]; onSelect: (event: ActivityEvent) => void }) {
  return <ItemGroup>{events.map((event, index) => <Item className="cursor-pointer" key={`${event.sequence ?? event.timestamp}-${event.kind}-${index}`} role="button" tabIndex={0} variant="outline" onClick={() => onSelect(event)} onKeyDown={(value) => { if (value.key === "Enter" || value.key === " ") onSelect(event) }}><ItemContent><ItemHeader><ItemTitle className="min-w-0 font-mono">{activityTitle(event)}</ItemTitle>{event.status ? <StatusBadge status={event.status} /> : null}</ItemHeader><ItemDescription>{event.message || [event.kind, event.source, tunnelLabel(event), event.workspace_id].filter(Boolean).join(" · ")}</ItemDescription><div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted-foreground"><span>{formatTime(event.timestamp)}</span><span>{event.kind}</span>{event.duration_ms !== undefined ? <span>{formatDuration(event.duration_ms)}</span> : null}</div></ItemContent></Item>)}</ItemGroup>
}
...
function ActivityDetailContent({ event, unboundedRaw = false }: { event: ActivityEvent; unboundedRaw?: boolean }) {
  return <Tabs defaultValue="overview"><TabsList className="w-full"><TabsTrigger value="overview">Overview</TabsTrigger><TabsTrigger value="metadata">Metadata</TabsTrigger><TabsTrigger value="raw">Raw</TabsTrigger></TabsList><TabsContent className="mt-4 divide-y" value="overview"><DetailRow label="Status" value={event.status ? <StatusBadge status={event.status} /> : "-"} /><DetailRow label="Tool" value={event.tool || "-"} mono /><DetailRow label="Method" value={event.method || "-"} mono /><DetailRow label="Tunnel" value={tunnelLabel(event) || "-"} /><DetailRow label="Message" value={event.message || "-"} /><DetailRow label="Duration" value={event.duration_ms === undefined ? "-" : formatDuration(event.duration_ms)} /></TabsContent><TabsContent className="mt-4 divide-y" value="metadata"><DetailRow label="Call ID" value={event.call_id || "-"} mono /><DetailRow label="Sequence" value={event.sequence ?? "-"} mono /><DetailRow label="Timestamp" value={formatDateTime(event.timestamp)} /><DetailRow label="Source" value={event.source || "-"} mono /><DetailRow label="Tunnel ID" value={event.tunnel_id || "-"} mono /><DetailRow label="Workspace" value={event.workspace_id || "-"} mono /><DetailRow label="Session" value={event.session_hash || "-"} mono /><DetailRow label="Session access" value={event.session_access || "-"} mono /><DetailRow label="Session workspaces" value={event.session_workspace_count ?? "-"} mono /><DetailRow label="Received by" value={event.received_by_instance_id || "-"} mono /><DetailRow label="Executed by" value={event.executed_by_instance_id || "-"} mono /><DetailRow label="Kind" value={event.kind} mono /></TabsContent><TabsContent className="mt-4" value="raw"><JsonViewer maxHeight={unboundedRaw ? null : undefined} value={event.raw ?? event} /></TabsContent></Tabs>
}

function FilterSelect({ label, value, values, labels, onValueChange }: { label: string; value: string; values: string[]; labels?: Record<string, string>; onValueChange: (value: string) => void }) {
  return <Select value={value} onValueChange={onValueChange}><SelectTrigger className="w-full lg:w-44"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">{label}</SelectItem>{values.map((item) => <SelectItem key={item} value={item}>{labels?.[item] || item}</SelectItem>)}</SelectContent></Select>
}
...
function tunnelLabel(event: ActivityEvent) { return event.tunnel_name || event.tunnel_id || "" }
