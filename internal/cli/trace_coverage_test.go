...
func commandTraceContracts() map[string]commandTraceContract {
	...
	add(commandTraceInstrumented, []string{"upstream.store.load.completed"}, "mcp server list", "mcp server show")
	add(commandTraceInstrumented, []string{"upstream.server.save.completed"}, "mcp server add", "mcp server configure", "mcp server disable", "mcp server enable")
	add(commandTraceInstrumented, []string{"upstream.server.remove.completed"}, "mcp server remove")
	add(commandTraceInstrumented, []string{"upstream.status.list.completed"}, "mcp server status")
	add(commandTraceInstrumented, []string{"upstream.tools.discover.completed", "upstream.tools.list.completed"}, "mcp server tools")
	add(commandTraceInstrumented, []string{"oauth.login.completed"}, "mcp server auth login")
	add(commandTraceInstrumented, []string{"oauth.store.delete.completed"}, "mcp server auth logout")
	add(commandTraceInstrumented, []string{"oauth.store.status.completed"}, "mcp server auth status")
