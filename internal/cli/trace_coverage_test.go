...
func commandTraceContracts() map[string]commandTraceContract {
	...
	add(commandTraceInstrumented, []string{"auth.token.rotate.completed"}, "auth admin create", "auth mcp create", "auth mcp rotate")
	...
	add(commandTraceInstrumented, []string{"auth.status.completed"}, "auth status", "auth mcp status")
	add(commandTraceTrivial, nil, "auth mcp show", "auth mcp copy")
	...
	add(commandTraceTrivial, nil, "tunnel", "tunnel list", "tunnel status", "tunnel add", "tunnel attach", "tunnel update", "tunnel detach", "tunnel enable", "tunnel disable", "tunnel start", "tunnel stop")
	add(commandTraceTrivial, nil, "tunnel cf", "tunnel cf status", "tunnel cf start", "tunnel cf stop")
