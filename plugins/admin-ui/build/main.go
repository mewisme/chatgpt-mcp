...
func main() {
	...
	flag.StringVar(&sourceRoot, "source-root", "", "built Admin UI root")
	flag.StringVar(&templatePath, "template", "plugins/admin-ui/plugin.json", "plugin manifest template")
...
func build(sourceRoot, templatePath, outputRoot string) (string, string, error) {
	...
	artifact, ok := manifest.Platforms["any/any"]
	if !ok {
		return "", "", errors.New("plugin template is missing any/any")
	}
	...
	if err != nil {
		return "", "", fmt.Errorf("admin UI entrypoint is unavailable: %w", err)
	}
	if !info.Mode().IsRegular() {
		return "", "", errors.New("admin UI entrypoint is not a regular file")
	}
	...
	manifest.Platforms["any/any"] = artifact
...
func deterministicZip(sourceRoot, outputPath string, extras []licenseinventory.ExtraFile) error {
	...
	err := filepath.WalkDir(sourceRoot, func(path string, entry fs.DirEntry, walkErr error) error {
		...
		if info.Mode()&os.ModeSymlink != 0 || !entry.IsDir() && !info.Mode().IsRegular() {
			return fmt.Errorf("unsupported Admin UI entry type: %s", path)
		}
	...
	sort.Strings(paths)
	temp, err := os.CreateTemp(filepath.Dir(outputPath), ".admin-ui-plugin-*.zip")
	if err != nil {
		return err
	}
	tempPath := temp.Name()
