...
func TestRequestActivityToolCallRemainsMCPRequest(t *testing.T) {
	...
	if event.Kind != "mcp_request" || event.Method != "tools/call" || event.Tool != "read_text_file" || event.WorkspaceID != "ws_123" || event.Status != "ok" || event.DurationMS != 12 {
