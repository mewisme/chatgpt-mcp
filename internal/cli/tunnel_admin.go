...
func tunnelAdminKeySetCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long:  "Verify Tunnels Manage access by listing an organization, workspace, or tenant scope, then store the admin key and verification scope in tunnel.<ext>. If no scope flag is provided, cgm first reuses a stored scope or derives one from the currently configured tunnel metadata.",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			log.Detail("secret", config.TunnelSecretPath())
			return nil
		},
	}
